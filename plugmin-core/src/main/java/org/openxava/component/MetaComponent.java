...
public class MetaComponent implements Serializable {
	...
	public MetaDropDown getMetaDropDown(String name) throws Exception {		
		if (Is.emptyString(name)) return getMetaDropDown();
		if (metaDropDowns == null) {			
			throw new ElementNotFoundException("dropdown_not_found", name, getName());
		}
		MetaDropDown result = (MetaDropDown) metaDropDowns.get(name);
		if (result == null) {
			throw new ElementNotFoundException("dropdowns_not_found", name, getName());
		}
		return result;
	}
	...
	public Collection getMetaDropDowns() throws Exception{
		Collection metaDropDowns = new ArrayList();
		metaDropDowns.add(getMetaDropDown());
		if (this.metaDropDowns != null) metaDropDowns.addAll(this.metaDropDowns.values());
		return metaDropDowns;
	}
	
	private void setMetaDropDown(MetaDropDown metaDropDown) {
		if (this.metaDropDown != null) {			
			throw new XavaException("no_more_1_dropdown", getName());
		}
	...
	public void addMetaDropDown(MetaDropDown metaDropDown) throws XavaException {
		...
		if (Is.emptyString(name)) { // by default
			setMetaDropDown(metaDropDown);
		}
		else { // with name
			if (metaDropDowns == null) {
				metaDropDowns = new HashMap();			
			}
