...
public class ReferenceConverterToDBListener implements PreInsertEventListener,
	...
	private static Log log = LogFactory
			.getLog(ReferenceConverterToDBListener.class);

	public boolean onPreInsert(PreInsertEvent ev) {
		if (!(ev.getEntity() instanceof IModel))
			return false;
	...
	private void applyConverter(IModel model, List propertyNames, Object[] state) {
		...
		try {
			...
			if (!mapping.hasReferenceConverters())
				return;
			Collection referenceMappings = mapping
					.getReferenceMappingsWithConverter();
			Iterator it = referenceMappings.iterator();
			while (it.hasNext()) {
				ReferenceMapping referenceMapping = (ReferenceMapping) it
						.next();
				if (mapping
						.isReferenceOverlappingWithSomeProperty(referenceMapping
								.getReference()))
					continue;
				PropertiesManager pm = new PropertiesManager(model);
				Object referencedObject = pm.executeGet(referenceMapping
						.getReference());
				MetaReference metaReference = metaModel
						.getMetaReference(referenceMapping.getReference());
				currentReference = metaReference.getName();
				if (referencedObject == null) {
					referencedObject = metaReference.getMetaModelReferenced()
							.getPOJOClass().newInstance();
				}

				Collection detailMappings = referenceMapping.getDetails();
				Iterator itd = detailMappings.iterator();
				while (itd.hasNext()) {
					ReferenceMappingDetail referenceMappingDetail = (ReferenceMappingDetail) itd
							.next();
					if (referenceMappingDetail.hasConverter()) {
						IConverter conv = referenceMappingDetail.getConverter();
						PropertiesManager pm2 = new PropertiesManager(
								referencedObject);
						Object propertyValue = pm2
								.executeGet(referenceMappingDetail
										.getReferencedModelProperty());
						pm2.executeSet(referenceMappingDetail
								.getReferencedModelProperty(), conv
								.toDB(propertyValue));
					}
				}
				int i;
			...
			throw new HibernateException(XavaResources.getString(
					"generator.conversion_error", currentReference,
					model.getClass(), ""));
		}
	}
